...
def calculate_churn_probability_common(customer):
    ...
    # 나이별 위험도 => age과 churned의 상관관계가 없으므로 이 항목은 제외해야 한다고 판단됨
    # if customer['age'] < 25:
    #     base_probability += 15
    ...
    if customer['subscription_type'] == 'Basic':
        ...
        base_probability -= 10
        # base_probability += 5 # Standard의 이탈 위험히 Premium보다 새발의 피로 높은 것으로 판단돼 다음과 같이 수정 요함
    
    # 결제 방법별 위험도
    # if customer['payment_method'] == 'Gift Card': # Crypto도 이탈 위험이 비슷하므로 추가해야함
    if customer['payment_method'] in ['Gift Card', 'Crypto']:
        base_probability += 25
    elif customer['payment_method'] == 'Credit Card':
        # base_probability -= 10
        base_probability -= 20 # Gift Card, Crypto의 이탈여부 격차에 비해 살짝 낮으므로 다음과 같이 수정필요
    # elif customer['payment_method'] == 'PayPal': # Debit Card까지 포함해야함
    else:
        # base_probability -= 5
        base_probability -= 15 # 이탈 여부 격차에 따른 조정
    ...
    if customer['last_login_days'] > 30:
        ...
        # base_probability += 15 # 14초과 30이하에서는 이탈비율이 절반에 미치지 않음
        base_probability -= 13
    elif customer['last_login_days'] < 3:
        # base_probability -= 10
        base_probability -= 30 # 3미만인 경우 이탈률이 상당히 낮음
    else: # 나머지 3이상 14이하도 추가 필요
        base_probability -= 25
    
    # 월 구독료별 위험도 => 상관계수가 last_login_days의 절반이므로 이에 따라 조정
    ...
    if customer['gender'] == 'Female':
        base_probability += 3
    # elif customer['gender'] == 'Other': # female이외의 성별에서는 상관관계가 없음
    #     base_probability += 5
...
if customer_id_input:
    ...
    if not customer_data.empty:
        ...
        if customer['device'] in ['Tablet', 'Laptop']:
            risk_factors.append("태블릿 및 랩탑의 사용 (불편한 시청 환경)")
        if customer['number_of_profiles'] == 1:
            risk_factors.append("단일 프로필 (가족 공유 미활용)")
        # 추가 제안
        if customer['favorite_genre'] == 'Drama':
            risk_factors.append("주 이용 콘텐츠가 드라마 (특정 드라마에 대한 실망)")
        ...
        if customer['payment_method'] in ['Credit Card', 'Debit Card', 'PayPal']:
            protection_factors.append("신용카드 등 결제수단 이용 (안정적 결제)")
