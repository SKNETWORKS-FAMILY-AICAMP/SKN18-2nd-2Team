class DataProcessing():
    ...
    def __init__(self, train_csv, test_csv, seed = 42, test_size = 0.2):
        ...
        self.train_final = pd.DataFrame(self.scaler.fit_transform(self.train_merge), columns=self.train_merge.columns)
        self.test_final = pd.DataFrame(self.scaler.transform(self.test_merge), columns=self.test_merge.columns)
        self.test_df_final = pd.DataFrame(self.scaler.transform(self.test_df_merge), columns=self.test_df_merge.columns)
